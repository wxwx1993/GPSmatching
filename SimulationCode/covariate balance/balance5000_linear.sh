...
#SBATCH --array=0-3 #number of jobs to run, it is currently set to 1 job(1 year), change it to array=1-13 for 13 years(jobs)
#SBATCH --mem=300000 #memory requested
#SBATCH -J balance5000  #job name, this case:aw-area weighted aggregation
...
R CMD BATCH --quiet --no-restore --no-save balance5000_linear.R /n/home01/xwu1993/GPSmatching/output/balance5000_linear_${SLURM_ARRAY_TASK_ID}.Rout
