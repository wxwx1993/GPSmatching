...
2) We implement a nested-loop algorithm, with $l$ in $1,2,\ldots, L$ as the outer-loop, and $j'$ in $1 ,\ldots,N$ as the inner-loop. The algorithm outputs the final product of our design stage, i.e., a matched set with $N\times L$ units. \
		\textbf{for} $l = 1,2,\ldots, L$ \textbf{do} \
		 Choose  \textbf{one} exposure level of interest $w^{(l)} \in  \{w^{(1)}, w^{(2)}, ..., w^{(L)}\}$. \
		...
		\setlength{\leftskip}{0pt}
			...
			2.2 Implement the matching to find \textbf{an} observed unit -- denoted by $j$ -- that matched with $j'$ with respect to \underline{both} the exposure $w_{j}\approx w^{(l)}$ and the estimated GPS $\hat{e}(w_j, \mathbf{c}_{j}) \approx e^{(l)}_{j'}$ (under a standardized Euclidean transformation). 
	...
	\text{} \hspace{0.5cm} \textbf{end for}
	\begin{itemize}
	\text{} \hspace{0.5cm} \item[] Note: We allow multiple $j'$ (e.g., $j' =1$ and $j' = 5$) to be matched with the same observed unit $j$ throughout the inner-loop $j'$ in $1 ,\ldots,N$ (``matching with replacement").
	\end{itemize}
	\textbf{end for}
