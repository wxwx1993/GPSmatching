...
$Corr [\mathbf{c}_{i},  w_{i}] \approx \mathbf{0}.$
We calculate the absolute correlation in the pseudo population dataset as  
\begin{align*}
 \big\lvert \sum_{i=1}^{N\times L}  \tilde{\mathbf{c}}_{i}  \tilde{w}_{i} \big\lvert,
\end{align*}
where $\tilde{}$ indicates centered and scaled variables.
...
\begin{align*}
\overline{\big\lvert \sum_{i=1}^{N\times L} \tilde{\mathbf{c}}_{i}  \tilde{w}_{i} \big\lvert} < \boldsymbol{\epsilon}_1.
\end{align*}
