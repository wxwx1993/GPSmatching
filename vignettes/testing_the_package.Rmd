...
Use `devtools::install_github` to install the package. If you do not specify the `ref`, it will install the master (or main) branch. The `master` branch hosts the latest released code. The latest updates are committed to the `develop` branch. Read [Git Branching Model](contribution.Rmd) for more details. 
...
Forking is explained in the [Contribution](contribution.md) page.
