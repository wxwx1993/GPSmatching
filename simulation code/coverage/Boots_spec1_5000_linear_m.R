...
parameters <- expand.grid(1:500,c(1:10))[(process+1),]

m <- as.numeric(parameters[2])
spec = 1
cova_spec = 1
sample_size = 5000
delta_n <- 0.4
...
## Bootstrap
  ...
  set.seed(boots_id)
  subset <- simulated_data[sample(1:sample_size,floor(m*sqrt(sample_size)),replace=T), ]
...
ci_coverage <- sapply(1:100, function(i) {
  ...
  wald_ci <- c(erf$erf[i] - 1.96*sd(match_coefs_boots[i,], na.rm = T) *sqrt(m*sqrt(sample_size))/sqrt(sample_size),
        erf$erf[i] + 1.96*sd(match_coefs_boots[i,], na.rm = T) *sqrt(m*sqrt(sample_size))/sqrt(sample_size))
...
saveRDS(ci_coverage,
     file=paste0("/n/home01/xwu1993/GPSmatching2/coverage/5000_linear_m/coverage",sample_size,"gps_spec",spec,"cova_spec",cova_spec,"m",m,"process",process, ".rds"))
